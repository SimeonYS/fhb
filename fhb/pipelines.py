...
class FhbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('fhb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `fhb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM fhb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO fhb (date, title, link, content)"
